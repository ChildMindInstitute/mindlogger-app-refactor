...
echo "[Unity][Unlink react-native-unity] Uncommenting in source_files from podspec ..."
...
echo "[Unity][Unlink react-native-unity] Uncommenting in vendored_frameworks from podspec ..."
sed '/^[[:space:]]*#[[:space:]]*s\.vendored_frameworks/ s/^\([[:space:]]*\)#[[:space:]]*/\1/' \
  $1/react-native-unity.podspec > $1/react-native-unity.podspec.tmp \
  && mv $1/react-native-unity.podspec.tmp $1/react-native-unity.podspec

echo "[Unity][Unlink react-native-unity] Enabling android build support ..."
if [ -d $1/android.disabled ]; then mv $1/android.disabled $1/android; fi
