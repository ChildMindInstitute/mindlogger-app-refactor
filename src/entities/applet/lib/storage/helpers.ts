...
export const clearStorageRecords = {
  byEventId: (eventId: string) => {
    const activityRecordKeys = activityStorage.getAllKeys().filter((keys) => keys.includes(eventId));

    const flowRecordKeys = flowStorage.getAllKeys().filter((keys) => keys.includes(eventId));
