...
const LineChart: FC<Props> = ({ data, config }) => {
  ...
  const getXAxisDots = (): Array<ChartAxisDot> => range(8).map((item) => ({ dot: item, value: 0 }));

  const getYAxisDots = (): Array<ChartAxisDot> => range(maxValue + 1).map((item) => ({ dot: 1, value: item * 2 }));

  const numberValueExists = (value: number | null) => !!value || typeof value === 'number';
  ...
  const lineChartData: Array<LineChartDataItem> = useMemo(() => {
    ...
    return currentWeekDates.flatMap((currentWeekDate) => {
      const currentWeekDayValues = data.map((dataItem) =>
        areDatesEqual(dataItem.date, currentWeekDate) && numberValueExists(dataItem.value) && dataItem.value! < maxValue
          ? dataItem.value! * 2
          : null,
      );
  ...
  return (
    <Box>
      <VictoryChart>
        ...
        <VictoryAxis style={{ axis: { stroke: colors.lightGrey } }} dependentAxis tickFormat={() => null} />
        ...
        <VictoryLine style={{ data: { stroke: colors.primary } }} data={lineChartData} x="date" y="value" />

        <VictoryScatter style={{ data: { fill: colors.primary } }} data={lineChartData} x="date" y="value" size={5} />
      </VictoryChart>
    </Box>
  );
};
